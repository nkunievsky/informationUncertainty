...
class infomrationValueClass():
    def __init__(self,dataList,targetAge=40*12,maxAge=70*12, beta=0.995,R=1.005,flowUtility=False,
    ...
    def dataValidation(self):
        ...
        '''
        numCols = self.balancedData.shape[1]-2
        assets = self.balancedData[:,1] 
        incomeCols = self.balancedData[:,2+np.arange(0,numCols,2)]
        consumptionCols = self.balancedData[:,2+np.arange(1,numCols,2)]
    ...
    def createBalancedPanel(self):
        ...
        for d in self.finData:
            ...
            FutureIncomeConsumptionStream = cSample[:,2+(self.targetAge -cAge)*2:]
            #Aggregate the past into the assets
            incomeConsumptionStream_upToTargetAge =  cSample[:,2:(1+self.targetAge-cAge)*2] #The plus 1 here is because we include the current age
            ...
            dataForUncEstimation = np.concatenate((age,assets,FutureIncomeConsumptionStream),axis=1)
            '''
            Impose some restriction for solver - should think about this more carefully 
            trimVar = 1e7
            dataForUncEstimation[dataForUncEstimation>trimVar]= trimVar
            '''            
        ...
        # Update object 
        self.initalCapital = self.balancedData[:,1] 
        incomeConsumptionStreams = self.balancedData[:,2:]
